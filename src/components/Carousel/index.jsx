...
const Carousel = (props) => {
	...
	return (
		<div className={styles.carousel}>
			<div
				style={{ paddingBottom: `${props.aspectRatio[1] / props.aspectRatio[0] * 100}%` }}
				role="presentation"
			/>
			{slides}
			{slides.length > 1 && (
				<>
					<CarouselPrevButton onClick={() => { setIndex(index - 1 !== -1 ? index - 1 : slides.length - 1) }} />
					<CarouselNextButton onClick={() => { setIndex(index + 1 !== slides.length ? index + 1 : 0) }} />
				</>
			)}
		</div>
	);
